...
class VoxelNet(nn.Module):
    ...
    def forward(self, example, detection_2d_path):     #注意，这里的example是重新组织后的input
    ...
    def train_stage_2(self, example, preds_dict, top_predictions):
        ...
        # print("batch_anchors.shape:\n", batch_anchors.shape)    #[1, 107136, 7]
        # print("batch_anchors:\n", batch_anchors)
        ########################
        ...
        """
        batch_anchors:
         tensor([[[  0.1600, -39.5200,  -1.7800,  ...,   3.9000,   1.5600,   0.0000],
         [  0.1600, -39.5200,  -1.7800,  ...,   3.9000,   1.5600,   1.5700],
         [  0.4800, -39.5200,  -1.7800,  ...,   3.9000,   1.5600,   0.0000],
         ...,
         [ 68.6400,  39.5200,  -1.7800,  ...,   3.9000,   1.5600,   1.5700],
         [ 68.9600,  39.5200,  -1.7800,  ...,   3.9000,   1.5600,   0.0000],
         [ 68.9600,  39.5200,  -1.7800,  ...,   3.9000,   1.5600,   1.5700]]],
       device='cuda:0')
        ...
        #print("example['image_shape']:\n", example[15])      #例：[[ 375 1242]]

        ############################################
        #MX这里anchors_mask是什么意思？CLOCs中是None，这里不是
        ############################################
        ...
        if self._use_direction_classifier:                                      #True
            ...
            batch_dir_preds = batch_dir_preds.view(batch_size, -1, 2)           #shape: [1, 70400, 2]
            ########################
            # 讨巧的方法，先取前70400个套用原来的代码
            ########################
        ...
        for box_preds, cls_preds, dir_preds, rect, Trv2c, P2, img_idx, a_mask in zip(       #最外层是1，没有循环，只执行一次
            ...
            t5_1 = time.time()
            #print("t51 - t5 time: ", (t5_1 - t5)*1000)              #0.04ms
